...
class AsyncMerchantClient(BaseMerchantClient):
    def __init__(
            self,
        ...
        self._http_client = http_client or AiohttpHttpClient()
    ...
    async def _send_request(
        ...
        _, response_data = await self._http_client.send_request(
            method=method,
            url=uri,
            data=data,
            headers=headers or {}
    ...
    async def get_exchange_balances(
        ...
        response = await self._send_request('GET', uri, headers=headers)
        return self._process_response(response, mc_dto.TotalExchangeBalanceResponse)

    async def get_external_wallet(
            self,
        ...
        response = await self._send_request('POST', uri, data=data, headers=headers)
        return self._process_response(response, mc_dto.ExternalAddressesResponse)

    async def get_processing_wallets_balances(
            self, x_api_key: Optional[str] = None, host: Optional[str] = None
    ...
    async def get_store_currencies(
            self, x_api_key: Optional[str] = None, host: Optional[str] = None
    ...
    async def get_store_currency_rate(
    ...
    async def get_withdrawal_processing_status(
    ...
    async def get_hot_wallet_balances(
            self, x_api_key: Optional[str] = None, host: Optional[str] = None
    ...
    async def delete_withdrawal_from_processing(
            self, withdrawal_id: str, x_api_key: Optional[str] = None, host: Optional[str] = None
    ) -> None:
        host, x_api_key = self._get_actual_request_params(x_api_key, host)
        uri = f"{host}/api/v1/external/withdrawal-from-processing/{withdrawal_id}"
